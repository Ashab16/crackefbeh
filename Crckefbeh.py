...
def banner():
	clear()
	wel = '# WELCOME TO RIOXPLOIT TOOLS'
...
def passwrd():
	ler = '# SEDANG MEMBELOKAN TETE, TEKAN CTRL+Z UNTUK BERHENTI'
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['nama1234','sayangku','sayang123','bismillah','anjing','katasandi','sandi123']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
{B} |
{P}[•] AUTHOR : RIO XPLOIT
{P}[•] WHATSAPP : 083184311442
{B}[•] FACEBOOK : gak punya """)
def __key__ ():#line:42
